...
export function ClickCollectSelector() {
      ...
      (error) => {
        ...
        switch (error.code) {
          case error.PERMISSION_DENIED:
            setLocationError(
              "Geolocation permission denied. Please enter an address."
            );
            break;
          case error.POSITION_UNAVAILABLE:
            setLocationError(
              "Location unavailable. Please enter an address."
            );
            break;
          case error.TIMEOUT:
  ...
  const handleDemoLocation = useCallback(async () => {
    ...
    setLocationSource("address");
    setSelectedAddress("Milan Center (Demo)");
      ...
      <SheetContent side="right" className="w-full sm:max-w-2xl lg:max-w-4xl overflow-y-auto p-0" showCloseButton={false}>
        <SheetHeader className="p-6 pb-0">
          <div className="flex items-center gap-2">
            <SheetTitle className="flex-1 text-lg">Click & Collect</SheetTitle>
        ...
        <div className="p-6 space-y-6">
              ...
              <div className="space-y-4">
                  ...
                  <>
                    ...
                    <div className="flex items-center justify-between gap-3 px-4 py-3 bg-muted/50 rounded-lg">
                      <div className="flex-1">
                        <p className="text-sm font-medium">Only products available here</p>
                        <p className="text-xs text-muted-foreground">
                          Show only products in stock at {currentShop.name}
                        </p>
                      </div>
                      <Switch
                ...
                {!currentShop && (
                  <div className="p-3 bg-muted/50 rounded-lg">
                    <p className="text-sm">
                      <span className="font-medium">{filteredNearbyShops.length}</span> stores found within{" "}
                      <span className="font-medium">{searchRadiusKm} km</span>
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">
                      Select a store to filter the catalog, or browse by distance
                    </p>
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-4">
                  ...
                  <div className="h-[300px] rounded-lg overflow-hidden">
                    <LocationMap
                      ...
                      onLocationSelect={async (loc) => {
                        setUserLocation(loc);
                        setLocationSource("address");
                        setSelectedAddress("Location on map");
                        await fetchShopsByLocation(loc);
                      }}
                  ...
                  <div className="h-[300px] flex flex-col">
                    <p className="text-sm font-medium mb-2">
                      {currentShop ? "Change store:" : "Select a store:"}
                    </p>
                    <div className="flex-1 overflow-y-auto space-y-2 pr-1">
                      {filteredNearbyShops.length === 0 ? (
                        <p className="text-sm text-muted-foreground text-center py-4">
                          {serviceFilter ? "No stores with this service" : "No stores found"}
                        </p>
                      ) : (
          ...
          <section className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-medium">When do you want to collect?</h3>
                <p className="text-sm text-muted-foreground">
            ...
            <details className="group">
              ...
              <div className="space-y-3 mt-3">
                <div className="grid grid-cols-2 gap-2">
                  {getQuickDateOptions()
                    .filter((option) => option.label !== "Today")
                    .map((option) => (
                      <Button
                        key={option.label}
                        variant={
