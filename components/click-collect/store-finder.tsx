...
export function StoreFinder() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold mb-2">Store Finder</h1>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        {serviceFilter && shopsToShow.length > 0 && (
          <span className="inline-flex items-center px-2 py-1.5 text-xs text-muted-foreground">
            {shopsToShow.length} {shopsToShow.length === 1 ? "store" : "stores"}
          </span>
        )}
      </div>
        ...
        <div className="space-y-3 max-h-[500px] overflow-y-auto">
          ...
          {searchPerformed && !isLoadingShops && shopsToShow.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              {serviceFilter
                ? `No stores with ${SERVICE_CONFIG[serviceFilter].label.toLowerCase()} found nearby`
                : "No stores found"}
            </div>
          )}
